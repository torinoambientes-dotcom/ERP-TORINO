...
function FactoryDisplayContent() {
  ...
  // Medição da Janela para Auto-Fit Forçado
  ...
  // Cálculo da escala exata forçada para caber 100% da tela sem cortes (com 2% de margem de segurança contra TV overscan)
  const autoFitScale = useMemo(() => {
    const targetWidth = 1920;
    const targetHeight = 1080;

    const scaleX = (dimensions.width * 0.98) / targetWidth;
    const scaleY = (dimensions.height * 0.98) / targetHeight;

    const baseScale = Math.min(scaleX, scaleY);
    return Math.min(2.5, Math.max(0.3, Number((baseScale * scale).toFixed(3))));
  }, [dimensions, scale]);
  ...
  return (
    <div className="bg-slate-950 h-screen w-screen flex items-center justify-center relative overflow-hidden select-none">
        
        {/* Stage de 1920x1080 com Escala Auto-Fit Forçada */}
        <div 
          className="bg-slate-100 text-slate-900 flex flex-col relative overflow-hidden shadow-2xl rounded-xl"
          style={{
            width: 1920,
            height: 1080,
        ...
        >
          ...
          <header className="px-6 py-3.5 border-b-2 border-slate-200 bg-white flex justify-between items-center z-10 shadow-xs flex-shrink-0 gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200">
                  ...
                  >
                    <option value="0.75">75% (Muito Pequena)</option>
                    <option value="0.85">85% (TV HD Ajustada)</option>
                    <option value="0.95">95% (TV Média)</option>
                    <option value="1">100% (Auto-Fit Padrão)</option>
                    <option value="1.1">110% (Ampliada)</option>
                    <option value="1.2">120% (TV 4K)</option>
                  </select>
        ...
        {zoomNotice && (
          <div className="fixed bottom-6 right-6 z-50 bg-slate-900/90 text-white text-xs font-black px-4 py-2 rounded-xl shadow-xl backdrop-blur-sm border border-slate-700 animate-in fade-in slide-in-from-bottom-2">
