  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen p-8">
      <PageHeader
        ...
        description="Progresso da produção em tempo real."
        className="text-white [&>h1]:text-white [&>p]:text-gray-300"
      />
        ...
        >
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
            {marceneiros.map((_, index) => (
                <div key={index} className={`h-2 w-8 rounded-full transition-all ${index === currentSlide ? 'bg-primary' : 'bg-gray-600'}`} />
            ))}
          </div>
        </Carousel>
      ) : (
        <div className="flex items-center justify-center h-96">
          <p className="text-xl text-gray-400">Nenhum marceneiro encontrado para exibir.</p>
        </div>
      )}
    </div>
  );
